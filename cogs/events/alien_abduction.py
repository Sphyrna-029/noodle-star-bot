...
class AlienAbductionCog(commands.Cog):
    ...
    async def _apply_auto_loss(self, user_id, username, mention, send):
        ...
        battle = BattleState(
            ...
            dungeon_level=0,
            player_hp=1,
            player_max_hp=stats["max_hp"] if stats["max_hp"] is not None else BASE_HP,
            player_stamina=stats["current_stamina"] if stats["current_stamina"] is not None else BASE_STAMINA,
            player_max_stamina=stats["max_stamina"] if stats["max_stamina"] is not None else BASE_STAMINA,
            player_attack=0,
            player_defense=0,
            mob_hp=ALIEN_MOB.hp,
